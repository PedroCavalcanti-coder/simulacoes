...
{
    /// <summary>
    /// Inspector for <see cref="FlaskVolume"/>. The default list drawer for
    ...
    /// </summary>
    [CustomEditor(typeof(FlaskVolume))]
    [CanEditMultipleObjects]
    class FlaskVolumeEditor : Editor
    {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Bake Now"))
                {
                    foreach (Object t in targets)
                        ((FlaskVolume)t).BakeInitialContents();
                }
        ...
        void DrawContentsList()
        {
            for (int i = 0; i < initialContents.arraySize; i++)
            {
                ...
                var liquid = liquidProp.objectReferenceValue as LiquidDefinition;
        ...
        void DrawStackPreview(FlaskVolume flask)
        {
            if (initialContents.arraySize == 0)
                return;

            var entries = new List<(LiquidDefinition liquid, float mL)>();
            for (int i = 0; i < initialContents.arraySize; i++)
            {
                SerializedProperty element = initialContents.GetArrayElementAtIndex(i);
                var liquid = element.FindPropertyRelative("liquid").objectReferenceValue as LiquidDefinition;
            ...
            // Real stacking order: lower category density floats on top, matching LiquidVolumePro.
        ...
        void DrawTotals(FlaskVolume flask)
        {
            float total = 0f;
            for (int i = 0; i < initialContents.arraySize; i++)
            {
        ...
        void DrawLiveState(FlaskVolume flask)
        {
            EditorGUILayout.Space();
