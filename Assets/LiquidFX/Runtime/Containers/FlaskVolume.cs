...
namespace LiquidFX
{
    ...
    ///
    /// - Layered (LVP "Multiple" details): a stack of named <see cref="LiquidDefinition"/> liquids,
    ...
    ///   (LiquidVolumePro's own miscibility rule - see LiquidCategory). See
    ...
    public sealed class FlaskVolume : MonoBehaviour, ILiquidContainer
    {
        [System.Serializable]
        ...
        // Parallel to Volume.liquidLayers: which LiquidDefinition (if any) occupies each slot.
        ...
        LiquidDefinition[] slotLiquid;

        // Reused across RemoveTopML calls so draining a mixed-group layer allocates nothing.
        readonly int[] groupBuffer = new int[LiquidVolume.MAX_LAYERS];
        ...
        public LiquidDefinition TopLiquid
        {
            get
            {
        ...
        public float AddML(float millilitres, Color color)
        {
            ...
            // No LiquidDefinition identity on this path (see AddLayeredML for the real transfer).
            ...
            if (top < 0)
            {
                if (!warnedAboutUntrackedAdd)
                {
                    ...
                    Debug.LogWarning($"{name}: received {millilitres:0.#} mL with no LiquidDefinition and no " +
                        "existing layer to merge into, so it was dropped. Pour from a layered FlaskVolume " +
        ...
        /// <summary>
        /// Adds an identified liquid. Merges into a matching layer when possible (LiquidVolumePro
        /// itself blends same-density/miscible layers), otherwise stacks a new one. This is the
        /// entry point a layer-aware pour should call instead of <see cref="AddML"/>.
        /// </summary>
        public float AddLayeredML(float millilitres, LiquidDefinition liquidToAdd)
        {
            if (millilitres <= 0f)
                return 0f;
            ...
            // 1. Exact same liquid already has a slot - top it up.
            int exact = FindSlotWithDefinition(liquidToAdd);
            if (exact >= 0)
            {
                layers[exact].amount += MLToAmount(accepted, layers[exact].density);
                Volume.UpdateLayers(true);
                return accepted;
            }

            // 2. A free slot - occupy it. LiquidVolumePro merges same-density (same category)
            //    layers together on its own, so this also covers "same category, different liquid".
        ...
        /// <summary>
        /// Removes up to <paramref name="millilitres"/> from whatever is exposed at the top of the
        /// stack, never reaching into the layer below in one call - the caller loops if it needs
        /// more than one layer's worth. When the top is a miscible group of several liquids of the
        /// same category, all of them drain together, proportional to their own volume, so the
        /// blended colour fades out smoothly instead of the group snapping between its members.
        /// Returns the millilitres actually removed and the dominant liquid in that group (the one
        /// with the most volume, decided before the removal) for coloring the stream/splash.
        /// </summary>
        public float RemoveTopML(float millilitres, out LiquidDefinition liquid)
        {
            ...
            float groupDensity = layers[top].density;
            int groupCount = 0;
            float groupAmountTotal = 0f;
            for (int i = 0; i < layers.Length; i++)
            {
                if (slotLiquid[i] == null || layers[i].amount <= AmountEpsilon)
                    continue;
                if (!Mathf.Approximately(layers[i].density, groupDensity))
                    continue;

                groupBuffer[groupCount++] = i;
                groupAmountTotal += layers[i].amount;
            }

            if (groupCount == 0 || groupAmountTotal <= 0f)
                return 0f;

            float requestedAmount = MLToAmount(millilitres, groupDensity);
            float removedAmount = Mathf.Min(requestedAmount, groupAmountTotal);
            if (removedAmount <= 0f)
                return 0f;

            int dominant = groupBuffer[0];
            for (int k = 1; k < groupCount; k++)
                if (layers[groupBuffer[k]].amount > layers[dominant].amount)
                    dominant = groupBuffer[k];
            liquid = slotLiquid[dominant];

            for (int k = 0; k < groupCount; k++)
            {
                int i = groupBuffer[k];
                float share = layers[i].amount / groupAmountTotal;
                layers[i].amount -= removedAmount * share;
                if (layers[i].amount <= AmountEpsilon)
                    ReleaseSlot(i);
            }

            Volume.UpdateLayers(true);
            return AmountToML(removedAmount, groupDensity);
        }
        ...
        void EnsureLayeredArrays()
        {
            if (slotLiquid == null || slotLiquid.Length != LiquidVolume.MAX_LAYERS)
                slotLiquid = new LiquidDefinition[LiquidVolume.MAX_LAYERS];
        }
        ...
        public void BakeInitialContents()
        {
            ...
            for (int i = 0; i < initialContents.Count && i < layers.Length; i++)
            {
                LayerCharge charge = initialContents[i];
                if (charge.liquid == null || charge.millilitres <= 0f)
                    continue;

                slotLiquid[i] = charge.liquid;
                charge.liquid.ApplyTo(ref layers[i]);
                layers[i].amount = MLToAmount(charge.millilitres, charge.liquid.Density);
            }
        ...
        /// <see cref="LiquidDefinition"/> behind them. They stay renderable and pourable - the slot
        ...
        int FindSlotWithDefinition(LiquidDefinition definition)
        {
            for (int i = 0; i < slotLiquid.Length; i++)
                if (slotLiquid[i] == definition)
                    return i;
            return -1;
        }
